...
angleTest:
	...
	add	r11, r4, #2		@increment x coordinate of the testing pixel by 2
	add	r6, r6, #2		@increment the temporary ball x coordinate by 2
	b	wallTest			

left45:
	sub	r11, r4, #2		@decrement x coordinate of the testing pixel by 2
	sub	r6, r6, #2		@decrement the temporary ball x coordinate by 2
	b	wallTest
...
bottomTest:
	cmp	r12, #824		@check is the next value of the y-coordinate for testing pixel is less than the floor (824)
	bge	floorBody		@if yes, fall through to ceilingAngleTest, otherwise, check for a brick collision
